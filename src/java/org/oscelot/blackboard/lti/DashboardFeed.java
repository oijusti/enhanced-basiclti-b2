...
public class DashboardFeed {
    ...
    private void getFeed() {
        ...
        if (hasContent) {
            ...
            String key = this.b2Context.getUser().getUserName() + "-" + this.tool.getId();
            ...
            if (contentType.equals(Constants.CONTENT_TYPE_RSS)) {
                ...
                int pos = data.toLowerCase(Locale.ENGLISH).indexOf("<body");
                if (pos >= 0) {
                    data = data.substring(pos);
                    pos = data.toLowerCase(Locale.ENGLISH).indexOf(">");
                    if (pos >= 0) {
                        data = data.substring(pos + 1);
                    } else {
                        data = "";
                    }
                    pos = data.toLowerCase(Locale.ENGLISH).indexOf("</body");
                    if (pos >= 0) {
    ...
    private void saveCustomParameters(Document xmlDoc) {
        ...
        while (iter.hasNext()) {
            ...
            if (item2 != null) {
                ...
                for (Iterator<Element> iter2 = (Iterator<Element>) item2.getDescendants(elementFilter); iter2.hasNext();) {
            ...
            try {
                if (params.isEmpty()) {
                    bbSession.removeGlobalKey(name + n);
                } else {
                    bbSession.setGlobalKey(name + n, Utils.mapToString(params));
                }
