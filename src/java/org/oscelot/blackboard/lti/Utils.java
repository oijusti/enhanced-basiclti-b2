...
public class Utils {
    ...
    private static final char[] HEX_CHARS = {'0', '1', '2', '3',
        ...
        '8', '9', 'a', 'b',
        'c', 'd', 'e', 'f',};
    private static volatile Comparator<PortalRole> cmSortByName = null;
...
// Calculate the hash
        try {
            ...
            digest.reset();
            byte[] hashdata;
            hashdata = digest.digest(data.getBytes(StandardCharsets.UTF_8));
            if (asHex) {
...
// Function to get a comma separated list of the LTI role names
    public static String getCRoles(String roleSetting) {
...
//        value = value.replaceAll("\\$Person.sms", "");
        ...
        if ((course != null) && value.contains("$Context.")) {
            ...
            if (props.getProperty("context_type").equals("CourseSection")) {
                if (tool.getDoSendContextSourcedid()) {
                    value = value.replaceAll("\\$Context.sourcedId", course.getBatchUid());
...
//                value = value.replaceAll("\\$Context.courseNumber", "");
//                value = value.replaceAll("\\$Context.credits", "");
//                value = value.replaceAll("\\$Context.maxNumberofStudents", "");
//                value = value.replaceAll("\\$Context.numberofStudents", "");
                value = value.replaceAll("\\$Context.dept", getPrimaryNode(course.getId(), true));
                value = value.replaceAll("\\$Context.timeFrame.begin",
                        formatCalendar(course.getStartDate(), Constants.ISO_DATE_FORMAT));
                value = value.replaceAll("\\$Context.timeFrame.end",
                        formatCalendar(course.getEndDate(), Constants.ISO_DATE_FORMAT));
//                value = value.replaceAll("\\$Context.enrollControl.accept", "");
//                value = value.replaceAll("\\$Context.enrollControl.allowed", "");
            }
        }
...
// Function to check if settings have moved to a new course
    public static void checkCourse(B2Context b2Context) {
        ...
        if (b2Context.hasCourseContext()) {
            String courseId = b2Context.getCourseIdAsString();
            ...
            if (oldCourseId.length() <= 0) {
                ...
                String toolOrder = courseContext.getSetting(false, true, "tools.order", courseContext.getSetting("tools.order", ""));
            ...
            if (tools != null) {
                try {
                    ...
                    List<Group> groups = groupDbLoader.loadByCourseId(courseContext.getCourseId());
                    Group group;
                    for (Iterator<Group> iter = groups.iterator(); iter.hasNext();) {
                        group = iter.next();
                        courseContext.setGroup(group);
                        doSave = false;
                        for (int i = 0; i < tools.length; i++) {
                            String tool = tools[i];
                            if (courseContext.getSetting(false, true, Constants.TOOL_PARAMETER_PREFIX + "." + tool + "." + Constants.TOOL_LINEITEM, null) != null) {
                                courseContext.setSetting(false, true, Constants.TOOL_PARAMETER_PREFIX + "." + tool + "." + Constants.TOOL_LINEITEM, null);
                                doSave = true;
                            }
                        }
...
// Function to check if grade centre column should be created
    public static boolean checkColumn(B2Context b2Context, String toolId, String toolName, String columnFormat,
        ...
        return checkColumn(b2Context, toolId, toolName, columnFormat, points, scorable, visible, create, null) != null;
    }

    public static Lineitem checkColumn(B2Context b2Context, String toolId, String toolName, String columnFormat,
            Integer points, boolean scorable, boolean visible, boolean create, Calendar dueDate) {
        ...
        if (lineitem != null) {
            ...
            if ((def != null) && (!toolName.equals(def.getTitle()) || ((dueDate == null) && (def.getDueDate() != null) || (dueDate != null) && (def.getDueDate() == null)
                    || ((dueDate != null) && def.getDueDate().compareTo(dueDate) != 0)))) {
                try {
                    def.setTitle(toolName);
                    if ((dueDate == null) || (dueDate.getTimeInMillis() != 0L)) {
    ...
    /**
     * Returns an ID value from a string representation of an ID.
     *
     * @param idString ID string value
     *
     * @return ID value
     */
    public static String getId(Id id) {

        String idString = "";
        if (id != null) {
            String[] idParts = id.toExternalString().split("_");
            if (idParts.length == 3) {
                idString = idParts[1];
            }
        }
...
// Function to encode any instances of the hash separator
    public static String encodeHash(String hash) {
        ...
        hash = hash.replace(Constants.HASH_SEPARATOR, "%" + arrayToHexString(Constants.HASH_SEPARATOR.getBytes(StandardCharsets.UTF_8)));
...
// Function to decode any instances of the hash separator
    public static String decodeHash(String hash) {
        ...
        hash = hash.replace("%" + arrayToHexString(Constants.HASH_SEPARATOR.getBytes(StandardCharsets.UTF_8)), Constants.HASH_SEPARATOR);
        hash = hash.replace("%25", "%");
...
// Function to extract a domain (including optional path) from a URL
    public static String urlToDomainName(String urlString) {
        ...
        try {
            ...
            urlString = urlString.toLowerCase(Locale.ENGLISH);
...
// Function to generate a new unique ID for a tool or domain
    public static String getNewToolId(B2Context b2Context, String toolName, boolean isDomain, boolean isSystemTool) {
        ...
        String baseName = toolName.toLowerCase(Locale.ENGLISH);
...
// Function to validatae the values for tool/domain configuration settings
    private static Map<String, String> checkXMLParams(B2Context b2Context, Map<String, String> params,
        ...
        extensionProps.put(Constants.TOOL_SIGNATURE_METHOD, Constants.DATA_SIGNATURE_METHOD_SHA1 + Constants.DATA_SIGNATURE_METHOD_SHA256
                + Constants.DATA_SIGNATURE_METHOD_SHA384 + Constants.DATA_SIGNATURE_METHOD_SHA512);
...
// Function to determine is a user has an available avatar
    ...
    public static String getResourceHandle(B2Context b2Context, String toolId) {
        ...
        String handle = "resource/x-" + b2Context.getVendorId().toLowerCase(Locale.ENGLISH) + "-" + b2Context.getHandle().toLowerCase(Locale.ENGLISH);
...
// Function to check if a module ID has been passed
    ...
    public static Node initNode(HttpSession session, B2Context b2Context, boolean systemNode) {
        ...
        return initNode(b2Context, systemNode, false);

    }

    public static Node initNode(B2Context b2Context, boolean systemNode, boolean includeRoot) {
